...
cgt_works_p (void)
{
  ...
  if (clock_gettime2 (CGT_ID, &unit) != 0)
    {
      if (speed_option_verbose)
  ...
  /* get the resolution */
  if (clock_getres2 (CGT_ID, &unit) != 0)
    {
      if (speed_option_verbose)
  ...
  if (cgt_unittime < 10e-9)
    {
      ...
      double duration;
      if (clock_gettime2 (CGT_ID, &start))
	{
	  if (speed_option_verbose)
	    printf ("clock_gettime id=%d error: %s\n", CGT_ID, strerror (errno));
	  result = 0;
...
speed_starttime (void)
{
  ...
  if (have_cgt && use_cgt)
    clock_gettime2 (CGT_ID, &start_cgt);
...
#define END_USE(name,value)                             \
  do {                                                  \
    if (speed_option_verbose >= 3)                      \
      printf ("speed_endtime(): used %s %lf\n", name, value);      \
    result = value;                                     \
    goto done;                                          \
  } while (0)
...
#define END_EXCEED(name,value)                                            \
  ...
  if (have_cgt    && use_cgt)     clock_gettime2 (CGT_ID, &end_cgt);
